...
import { BinsNews, DeleteBinsNews, PatchBinsNews } from '../../../services/adminServer'
            ...
            {news.map(item => (
                <div className="bins_news">
                    ...
                    <div className="bins_news_item">
                        <h4>{item.title}</h4>
                        <div className="btn btn-primary" onClick={() => handlePatch(item._id)}>Khôi phục</div>
                        <div className="btn btn-danger" onClick={() => handleDelete(item._id)}>Xoá vĩnh viễn</div>
                    </div>
                </div>
            ))}
        </div>
